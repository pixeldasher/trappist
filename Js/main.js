	...
	const main = function (_w, _d, _r, _b) {
		...
		const ariaFunction = (() => {
			...
			const getFocusElems = function (startingNode = _b) {
				return Array.from(startingNode.querySelectorAll(":any-link, button, input, select, textarea, [tabindex]")).filter((element) => {
					const unfocussable = "[disabled], [aria-disabled='true'], [aria-hidden='true']"
					if (!element.matches(`${unfocussable}, [tabindex='-1']`) && !element.closest(unfocussable)) return element
				})
			}
					...
					if (button instanceof HTMLButtonElement) {
						...
						button.addEventListener("click", () => {
							toggleMenu(controlElem, null, (/** @type {Boolean} */ openMenu) => {
								...
								if (haufeStellenFilter instanceof HTMLElement && haufeStellenFilter.contains(controlElem)) {
		...
		void (function parallaxFunction() {
			...
			const mediaParallax = function (element, index, media) {
				...
				media.style.transform = `translateZ(0) translateY(${((element.getBoundingClientRect().top + element.clientHeight / 2 + _r.clientHeight / 2) / _r.clientHeight - 1) * -parallaxMultiplier}%) scale(${100 + parallaxMultiplier / 2}%)`
